$(function(){
    ...
    var allPage=$("#maxPage").text();
    var reStatus=$("#selectRe").text();
    addPage(1,allPage);
    addEvent(allPage);
    if (document.title=="我的报修"){
        console.log(document.title);
        function getData(num){
            $('#table1').load("upPageForUs.do?curPage="+num);
            addPage(num,allPage);
            addEvent(allPage);
        }
    }else if (document.title=="查看报修"){
        console.log(document.title);
        function getData(num){
            $('#table1').load("upPageForSee.do?pageNum="+num+"&reStutas="+reStatus);
            addPage(num,allPage);
            addEvent(allPage);
        }
    }else if(document.title=="查看公告"){
        console.log(document.title);
        function getData(num){
            $('#table1').load("SeeAnnounPage.do?curPage="+num);
            addPage(num,allPage);
            addEvent(allPage);
        }
    }
